...
def get_context(context):
	# No server-side context needed — the page (recommendation_dashboard.js)
	# fetches data client-side via
	# stridenex_app.stridenex_app.ai_agent.recommendation_agent.get_recommendation_history
	pass


# Copyright (c) 2026, Stridenex App
# AI-powered internship / industry-project recommendation agent
# Backed by a self-hosted Ollama server (qwen2.5:1.5b) via a manual ReAct tool loop.
#
# WHY MANUAL REACT: Ollama's /api/generate endpoint has no native tool-calling
# (unlike Claude's Messages API). So instead we prompt the model to always
# answer in strict JSON describing which tool it wants to call, run that tool
# ourselves against real Frappe data, feed the result back as an "Observation",
# and repeat until it returns a final_answer action.
#
# SETUP REQUIRED:
#   1. Make sure the Ollama server is reachable from your bench:
#        curl http://135.181.6.215:11434/api/tags
#   2. Create the "AI Recommendation Log" DocType — JSON provided alongside
#      this file (ai_recommendation_log.json). Import it via
#      `bench --site <site> migrate` after placing it in your app's doctype folder.
#   3. (Optional) tune OLLAMA_URL / OLLAMA_MODEL below if these change.
#
# ASSUMPTIONS TO VERIFY (child table field names I couldn't see):
#   - Internship.required_skills rows expose a `skill` field.
#   - Student.skill rows expose `skill` and `level` fields.
#   - Student.career_interest rows expose a `career_interest` field.
#   Fix in _get_child_skill_names() / _build_student_profile() if different.
